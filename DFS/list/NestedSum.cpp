...
void traverseNestedList(NestedInteger& element, int currentDepth, int& depthSum) {
    vector<NestedInteger> nestedList = element.getList();
    currentDepth++;
    for (NestedInteger node : nestedList) {
        if (node.isInteger()) {
            depthSum += currentDepth * node.getInteger();
        } else {
            traverseNestedList(node, currentDepth, depthSum);
        }
    }
}
...
/**
 * https://www.hellointerview.com/community/questions/nested-list-sum/cm5eh7nrh04wf838ojjy6ed4e?level=SENIOR
 * 
 * Same principle as of DepthSumInverse - Traverse the list in DFS and calculate the depth sum (element * depth)
 */
...
int main() {
    ...
    cout << "Depth Sum Inverse : " << depthSumInverse(nestedList) << "\n";

    cout << "Depth Sum : " << depthSum(nestedList) << "\n";
